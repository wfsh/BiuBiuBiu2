...
namespace Sofunny.BiuBiuBiu2.Data {
    public partial class AbilityM_Grenade {
        public const string AbilityTypeID = AbilityConfig.AB_Grenade;
        public const ushort ConfigId = AbilityConfig.Grenade;
        private static Dictionary<byte, AbilityM_Grenade> idCache = new Dictionary<byte, AbilityM_Grenade>();
        private static Dictionary<string, AbilityM_Grenade> signCache = new Dictionary<string, AbilityM_Grenade>();
        private static List<AbilityM_Grenade> loadingCallbacks = new List<AbilityM_Grenade>();
        ...
        /// 格式：ID_主键值组合 = ID（如AbilityM_Grenade.ID_1）
        /// </summary>
        public const byte ID_Grenade = 1;
        public const byte ID_GrenadeTank = 2;

        /// <summary>
        /// CSV Sign 常量映射（自动生成，对应CSV的RowSign列）
        /// 格式：Sign_XXX
        /// </summary>
        public const string Sign_Grenade = "Grenade";
        public const string Sign_GrenadeTank = "GrenadeTank";

        /// <summary>
        /// ID查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_Grenade.ID_1）后调用Select()
        /// </summary>
        [NonSerialized]
        public byte K_ID = 0;
        /// <summary>
        /// Sign 查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_Grenade.Sign_XXX）后调用Select()
        /// </summary>
        [NonSerialized]
        ...
        public static void ClearCache() {
            ...
            IsCsvLoaded = false;
            isLoading = false;
            Debug.Log("已清除 AbilityM_Grenade 实例缓存");
        }
        ...
        private void DoSelectFromCache() {
            ...
            if (target != null) {
                ...
                Debug.LogError($"AbilityM_Grenade 查询未找到：K_Sign={this.K_Sign} 或 K_ID={this.K_ID} 或 K_RowSign={this.K_RowSign}");
            }
            callBack?.Invoke();
            callBack = null;
        }
        ...
        /// 多K字段匹配逻辑（遍历实例缓存找目标）
        /// </summary>
        private static AbilityM_Grenade FindByMultiKeys(string K_Sign) {
        ...
        private static void LoadCsvAndCacheAll(Action onLoadComplete) {
            ...
            AssetCsvManager.GetAbilityABAllCsvRows("AbilityM_Grenade", allRows => {
                if (allRows == null || allRows.Count == 0) {
                    Debug.LogError($"全量加载 CSV 失败：AbilityM_Grenade");
                ...
                foreach (var row in allRows) {
                    ...
                    if (!byte.TryParse(idStr, out byte rowId)) {
                        Debug.LogError($"CSV 行 ID 格式错误：AbilityM_Grenade → {idStr}");
                        continue;
                    }
                    if (idCache.ContainsKey(rowId)) {
                        Debug.LogError($"CSV AbilityM_Grenade 行 ID重复：{rowId}（已跳过）");
                        continue;
                    }
                    var rowSign = AssetCsvManager.GetCsvRowValue(row, "RowSign");
                    var instance = new AbilityM_Grenade();
                    ...
                    instance.M_IsHitBomb = AssetCsvManager.ParseValue<bool>(AssetCsvManager.GetCsvRowValue(row, "M_IsHitBomb"));
                    ...
                    instance.M_Speed = AssetCsvManager.ParseValue<float>(AssetCsvManager.GetCsvRowValue(row, "M_Speed"));
                ...
                IsCsvLoaded = true;
                #if UNITY_EDITOR
                Debug.Log($"CSV全量加载完成：AbilityM_Grenade → 缓存 {idCache.Count}个实例  （耗时 {Time.realtimeSinceStartup - time:F2}秒）");
                #endif
                onLoadComplete?.Invoke();
            });
        }
        ...
        /// </summary>
        private void CopyInstanceData(AbilityM_Grenade source) {
            // 拷贝主键字段（K_XXX）
            this.K_Sign = source.K_Sign;

            // 拷贝业务字段（M_XXX）
            this.M_EffectSign = source.M_EffectSign;
            this.M_IsHitBomb = source.M_IsHitBomb;
        ...
        /// 按主键创建实例（自动赋值K_主键字段）
        /// </summary>
        public static AbilityM_Grenade CreateForKey(string k_Sign) {
            // 缓存已加载时直接查询并返回缓存实例
            if (IsCsvLoaded) {
        ...
        /// 按ID创建实例（自动赋值K_ID）
        /// </summary>
        public static AbilityM_Grenade CreateForID(byte id) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        public static AbilityM_Grenade CreateForSign(string sign) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        private static AbilityM_Grenade CreateSO() {
#if UNITY_EDITOR
           var url = $"{AssetURL.AbilitySO}SO/AB/AbilityM_Grenade.asset";
           var so = AssetDatabase.LoadAssetAtPath<AbilityM_Grenade>(url);
           if (so != null && so.IsEditorUseSO) {
              so.IsSetCsvData = true;
              return so;
           }
#endif
           return new AbilityM_Grenade();
        }
    }
}
