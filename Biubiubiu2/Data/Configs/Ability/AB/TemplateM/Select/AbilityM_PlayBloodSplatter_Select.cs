...
namespace Sofunny.BiuBiuBiu2.Data {
    public partial class AbilityM_PlayBloodSplatter {
        public const string AbilityTypeID = AbilityConfig.AB_PlayBloodSplatter;
        public const ushort ConfigId = AbilityConfig.PlayBloodSplatter;
        private static Dictionary<byte, AbilityM_PlayBloodSplatter> idCache = new Dictionary<byte, AbilityM_PlayBloodSplatter>();
        private static Dictionary<string, AbilityM_PlayBloodSplatter> signCache = new Dictionary<string, AbilityM_PlayBloodSplatter>();
        private static List<AbilityM_PlayBloodSplatter> loadingCallbacks = new List<AbilityM_PlayBloodSplatter>();
        ...
        /// 格式：ID_主键值组合 = ID（如AbilityM_PlayBloodSplatter.ID_1）
        /// </summary>
        public const byte ID_BloodSplatter = 1;
        public const byte ID_WeaponFireGunHit = 2;
        public const byte ID_FlashgunHit = 3;
        public const byte ID_MachineGunHitOnce = 4;
        public const byte ID_JokerUAVSpadesHit = 5;
        public const byte ID_AceJokerFloatingGunHit = 6;
        public const byte ID_GoldJokerFloatingGunHit = 7;

        /// <summary>
        /// ID查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_PlayBloodSplatter.ID_1）后调用Select()
        /// </summary>
        [NonSerialized]
        public byte K_ID = 0;
        /// <summary>
        /// Sign 查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_PlayBloodSplatter.Sign_XXX）后调用Select()
        /// </summary>
        [NonSerialized]
        ...
        public static void ClearCache() {
            ...
            IsCsvLoaded = false;
            isLoading = false;
            Debug.Log("已清除 AbilityM_PlayBloodSplatter 实例缓存");
        }
        ...
        private void DoSelectFromCache() {
            AbilityM_PlayBloodSplatter target = null;
            // 1. 优先ID查询（性能最优，无遍历）
            if (this.K_ID != 0) {
            ...
            if (target != null) {
                ...
                Debug.LogError($"AbilityM_PlayBloodSplatter 查询未找到：K_Sign={this.K_Sign} 或 K_ID={this.K_ID} 或 K_RowSign={this.K_RowSign}");
            }
            callBack?.Invoke();
            callBack = null;
        }
        ...
        /// 多K字段匹配逻辑（遍历实例缓存找目标）
        /// </summary>
        private static AbilityM_PlayBloodSplatter FindByMultiKeys(string K_Sign) {
        ...
        private static void LoadCsvAndCacheAll(Action onLoadComplete) {
            ...
            AssetCsvManager.GetAbilityABAllCsvRows("AbilityM_PlayBloodSplatter", allRows => {
                if (allRows == null || allRows.Count == 0) {
                    Debug.LogError($"全量加载 CSV 失败：AbilityM_PlayBloodSplatter");
                ...
                foreach (var row in allRows) {
                    ...
                    if (!byte.TryParse(idStr, out byte rowId)) {
                        Debug.LogError($"CSV 行 ID 格式错误：AbilityM_PlayBloodSplatter → {idStr}");
                        continue;
                    }
                    if (idCache.ContainsKey(rowId)) {
                        Debug.LogError($"CSV AbilityM_PlayBloodSplatter 行 ID重复：{rowId}（已跳过）");
                        continue;
                    }
                    var rowSign = AssetCsvManager.GetCsvRowValue(row, "RowSign");
                    var instance = new AbilityM_PlayBloodSplatter();
                ...
                IsCsvLoaded = true;
                #if UNITY_EDITOR
                Debug.Log($"CSV全量加载完成：AbilityM_PlayBloodSplatter → 缓存 {idCache.Count}个实例  （耗时 {Time.realtimeSinceStartup - time:F2}秒）");
                #endif
                onLoadComplete?.Invoke();
            });
        }
        ...
        /// </summary>
        private void CopyInstanceData(AbilityM_PlayBloodSplatter source) {
            // 拷贝主键字段（K_XXX）
        ...
        /// 按主键创建实例（自动赋值K_主键字段）
        /// </summary>
        public static AbilityM_PlayBloodSplatter CreateForKey(string k_Sign) {
            // 缓存已加载时直接查询并返回缓存实例
            if (IsCsvLoaded) {
        ...
        /// 按ID创建实例（自动赋值K_ID）
        /// </summary>
        public static AbilityM_PlayBloodSplatter CreateForID(byte id) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        public static AbilityM_PlayBloodSplatter CreateForSign(string sign) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        private static AbilityM_PlayBloodSplatter CreateSO() {
#if UNITY_EDITOR
           var url = $"{AssetURL.AbilitySO}SO/AB/AbilityM_PlayBloodSplatter.asset";
           var so = AssetDatabase.LoadAssetAtPath<AbilityM_PlayBloodSplatter>(url);
           if (so != null && so.IsEditorUseSO) {
              so.IsSetCsvData = true;
              return so;
           }
#endif
           return new AbilityM_PlayBloodSplatter();
        }
    }
}
