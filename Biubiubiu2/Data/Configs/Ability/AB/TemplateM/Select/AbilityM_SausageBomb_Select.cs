...
namespace Sofunny.BiuBiuBiu2.Data {
    public partial class AbilityM_SausageBomb {
        public const string AbilityTypeID = AbilityConfig.AB_SausageBomb;
        public const ushort ConfigId = AbilityConfig.SausageBomb;
        private static Dictionary<byte, AbilityM_SausageBomb> idCache = new Dictionary<byte, AbilityM_SausageBomb>();
        private static Dictionary<string, AbilityM_SausageBomb> signCache = new Dictionary<string, AbilityM_SausageBomb>();
        private static List<AbilityM_SausageBomb> loadingCallbacks = new List<AbilityM_SausageBomb>();
        ...
        /// 格式：ID_主键值组合 = ID（如AbilityM_SausageBomb.ID_1）
        /// </summary>
        public const byte ID_Sausage = 1;

        /// <summary>
        /// ID查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_SausageBomb.ID_1）后调用Select()
        /// </summary>
        [NonSerialized]
        public byte K_ID = 0;
        /// <summary>
        /// Sign 查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_SausageBomb.Sign_XXX）后调用Select()
        /// </summary>
        [NonSerialized]
        ...
        public static void ClearCache() {
            ...
            IsCsvLoaded = false;
            isLoading = false;
            Debug.Log("已清除 AbilityM_SausageBomb 实例缓存");
        }
        ...
        private void DoSelectFromCache() {
            AbilityM_SausageBomb target = null;
            // 1. 优先ID查询（性能最优，无遍历）
            if (this.K_ID != 0) {
            ...
            if (target != null) {
                ...
                Debug.LogError($"AbilityM_SausageBomb 查询未找到：K_Sign={this.K_Sign} 或 K_ID={this.K_ID} 或 K_RowSign={this.K_RowSign}");
            }
            callBack?.Invoke();
            callBack = null;
        }
        ...
        /// 多K字段匹配逻辑（遍历实例缓存找目标）
        /// </summary>
        private static AbilityM_SausageBomb FindByMultiKeys(string K_Sign) {
        ...
        private static void LoadCsvAndCacheAll(Action onLoadComplete) {
            ...
            AssetCsvManager.GetAbilityABAllCsvRows("AbilityM_SausageBomb", allRows => {
                if (allRows == null || allRows.Count == 0) {
                    Debug.LogError($"全量加载 CSV 失败：AbilityM_SausageBomb");
                ...
                foreach (var row in allRows) {
                    ...
                    if (!byte.TryParse(idStr, out byte rowId)) {
                        Debug.LogError($"CSV 行 ID 格式错误：AbilityM_SausageBomb → {idStr}");
                        continue;
                    }
                    if (idCache.ContainsKey(rowId)) {
                        Debug.LogError($"CSV AbilityM_SausageBomb 行 ID重复：{rowId}（已跳过）");
                        continue;
                    }
                    var rowSign = AssetCsvManager.GetCsvRowValue(row, "RowSign");
                    var instance = new AbilityM_SausageBomb();
                ...
                IsCsvLoaded = true;
                #if UNITY_EDITOR
                Debug.Log($"CSV全量加载完成：AbilityM_SausageBomb → 缓存 {idCache.Count}个实例  （耗时 {Time.realtimeSinceStartup - time:F2}秒）");
                #endif
                onLoadComplete?.Invoke();
            });
        }
        ...
        /// </summary>
        private void CopyInstanceData(AbilityM_SausageBomb source) {
            // 拷贝主键字段（K_XXX）
        ...
        /// 按主键创建实例（自动赋值K_主键字段）
        /// </summary>
        public static AbilityM_SausageBomb CreateForKey(string k_Sign) {
            // 缓存已加载时直接查询并返回缓存实例
            if (IsCsvLoaded) {
        ...
        /// 按ID创建实例（自动赋值K_ID）
        /// </summary>
        public static AbilityM_SausageBomb CreateForID(byte id) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        public static AbilityM_SausageBomb CreateForSign(string sign) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        private static AbilityM_SausageBomb CreateSO() {
#if UNITY_EDITOR
           var url = $"{AssetURL.AbilitySO}SO/AB/AbilityM_SausageBomb.asset";
           var so = AssetDatabase.LoadAssetAtPath<AbilityM_SausageBomb>(url);
           if (so != null && so.IsEditorUseSO) {
              so.IsSetCsvData = true;
              return so;
           }
#endif
           return new AbilityM_SausageBomb();
        }
    }
}
