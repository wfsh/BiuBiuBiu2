...
namespace Sofunny.BiuBiuBiu2.Data {
    public partial class AbilityM_AncientDragonFlyFlame {
        public const string AbilityTypeID = AbilityConfig.AB_AncientDragonFlyFlame;
        public const ushort ConfigId = AbilityConfig.AncientDragonFlyFlame;
        private static Dictionary<byte, AbilityM_AncientDragonFlyFlame> idCache = new Dictionary<byte, AbilityM_AncientDragonFlyFlame>();
        private static Dictionary<string, AbilityM_AncientDragonFlyFlame> signCache = new Dictionary<string, AbilityM_AncientDragonFlyFlame>();
        private static List<AbilityM_AncientDragonFlyFlame> loadingCallbacks = new List<AbilityM_AncientDragonFlyFlame>();
        ...
        /// 格式：ID_主键值组合 = ID（如AbilityM_AncientDragonFlyFlame.ID_1）
        /// </summary>
        ...
        /// <summary>
        /// ID查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_AncientDragonFlyFlame.ID_1）后调用Select()
        /// </summary>
        [NonSerialized]
        public byte K_ID = 0;
        /// <summary>
        /// Sign 查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_AncientDragonFlyFlame.Sign_XXX）后调用Select()
        /// </summary>
        [NonSerialized]
        ...
        public static void ClearCache() {
            ...
            IsCsvLoaded = false;
            isLoading = false;
            Debug.Log("已清除 AbilityM_AncientDragonFlyFlame 实例缓存");
        }
        ...
        private void DoSelectFromCache() {
            AbilityM_AncientDragonFlyFlame target = null;
            // 1. 优先ID查询（性能最优，无遍历）
            if (this.K_ID != 0) {
            ...
            if (target != null) {
                ...
                Debug.LogError($"AbilityM_AncientDragonFlyFlame 查询未找到：K_BossType={this.K_BossType}, K_SceneType={this.K_SceneType} 或 K_ID={this.K_ID} 或 K_RowSign={this.K_RowSign}");
            }
            callBack?.Invoke();
            callBack = null;
        }
        ...
        /// 多K字段匹配逻辑（遍历实例缓存找目标）
        /// </summary>
        private static AbilityM_AncientDragonFlyFlame FindByMultiKeys(byte K_BossType, string K_SceneType) {
        ...
        private static void LoadCsvAndCacheAll(Action onLoadComplete) {
            ...
            AssetCsvManager.GetAbilityABAllCsvRows("AbilityM_AncientDragonFlyFlame", allRows => {
                if (allRows == null || allRows.Count == 0) {
                    Debug.LogError($"全量加载 CSV 失败：AbilityM_AncientDragonFlyFlame");
                ...
                foreach (var row in allRows) {
                    ...
                    if (!byte.TryParse(idStr, out byte rowId)) {
                        Debug.LogError($"CSV 行 ID 格式错误：AbilityM_AncientDragonFlyFlame → {idStr}");
                        continue;
                    }
                    if (idCache.ContainsKey(rowId)) {
                        Debug.LogError($"CSV AbilityM_AncientDragonFlyFlame 行 ID重复：{rowId}（已跳过）");
                        continue;
                    }
                    var rowSign = AssetCsvManager.GetCsvRowValue(row, "RowSign");
                    var instance = new AbilityM_AncientDragonFlyFlame();
                    ...
                    instance.M_FlyDisMax = AssetCsvManager.ParseValue<float>(AssetCsvManager.GetCsvRowValue(row, "M_FlyDisMax"));
                    instance.M_FlyDownTime = AssetCsvManager.ParseValue<float>(AssetCsvManager.GetCsvRowValue(row, "M_FlyDownTime"));
                    instance.M_FlySpeed = AssetCsvManager.ParseValue<float>(AssetCsvManager.GetCsvRowValue(row, "M_FlySpeed"));
                    instance.M_FlyTime = AssetCsvManager.ParseValue<float>(AssetCsvManager.GetCsvRowValue(row, "M_FlyTime"));
                    instance.M_FlyUpTime = AssetCsvManager.ParseValue<float>(AssetCsvManager.GetCsvRowValue(row, "M_FlyUpTime"));
                ...
                IsCsvLoaded = true;
                #if UNITY_EDITOR
                Debug.Log($"CSV全量加载完成：AbilityM_AncientDragonFlyFlame → 缓存 {idCache.Count}个实例  （耗时 {Time.realtimeSinceStartup - time:F2}秒）");
                #endif
                onLoadComplete?.Invoke();
            });
        }
        ...
        /// </summary>
        private void CopyInstanceData(AbilityM_AncientDragonFlyFlame source) {
            // 拷贝主键字段（K_XXX）
            ...
            this.M_FlyDisMax = source.M_FlyDisMax;
            this.M_FlyDownTime = source.M_FlyDownTime;
            this.M_FlySpeed = source.M_FlySpeed;
            this.M_FlyTime = source.M_FlyTime;
            this.M_FlyUpTime = source.M_FlyUpTime;
        ...
        /// 按主键创建实例（自动赋值K_主键字段）
        /// </summary>
        public static AbilityM_AncientDragonFlyFlame CreateForKey(byte k_BossType, string k_SceneType) {
            // 缓存已加载时直接查询并返回缓存实例
            if (IsCsvLoaded) {
        ...
        /// 按ID创建实例（自动赋值K_ID）
        /// </summary>
        public static AbilityM_AncientDragonFlyFlame CreateForID(byte id) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        public static AbilityM_AncientDragonFlyFlame CreateForSign(string sign) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        private static AbilityM_AncientDragonFlyFlame CreateSO() {
#if UNITY_EDITOR
           var url = $"{AssetURL.AbilitySO}SO/AB/AbilityM_AncientDragonFlyFlame.asset";
           var so = AssetDatabase.LoadAssetAtPath<AbilityM_AncientDragonFlyFlame>(url);
           if (so != null && so.IsEditorUseSO) {
              so.IsSetCsvData = true;
              return so;
           }
#endif
           return new AbilityM_AncientDragonFlyFlame();
        }
    }
}
