...
namespace Sofunny.BiuBiuBiu2.Data {
    public partial class AbilityM_AuroraDragonFireBall {
        public const string AbilityTypeID = AbilityConfig.AB_AuroraDragonFireBall;
        public const ushort ConfigId = AbilityConfig.AuroraDragonFireBall;
        private static Dictionary<byte, AbilityM_AuroraDragonFireBall> idCache = new Dictionary<byte, AbilityM_AuroraDragonFireBall>();
        private static Dictionary<string, AbilityM_AuroraDragonFireBall> signCache = new Dictionary<string, AbilityM_AuroraDragonFireBall>();
        private static List<AbilityM_AuroraDragonFireBall> loadingCallbacks = new List<AbilityM_AuroraDragonFireBall>();
        ...
        /// 格式：ID_主键值组合 = ID（如AbilityM_AuroraDragonFireBall.ID_1）
        /// </summary>
        ...
        /// <summary>
        /// ID查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_AuroraDragonFireBall.ID_1）后调用Select()
        /// </summary>
        [NonSerialized]
        public byte K_ID = 0;
        /// <summary>
        /// Sign 查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_AuroraDragonFireBall.Sign_XXX）后调用Select()
        /// </summary>
        [NonSerialized]
        ...
        public static void ClearCache() {
            ...
            IsCsvLoaded = false;
            isLoading = false;
            Debug.Log("已清除 AbilityM_AuroraDragonFireBall 实例缓存");
        }
        ...
        private void DoSelectFromCache() {
            AbilityM_AuroraDragonFireBall target = null;
            // 1. 优先ID查询（性能最优，无遍历）
            if (this.K_ID != 0) {
            ...
            if (target != null) {
                ...
                Debug.LogError($"AbilityM_AuroraDragonFireBall 查询未找到：K_BossType={this.K_BossType}, K_SceneType={this.K_SceneType} 或 K_ID={this.K_ID} 或 K_RowSign={this.K_RowSign}");
            }
            callBack?.Invoke();
            callBack = null;
        }
        ...
        /// 多K字段匹配逻辑（遍历实例缓存找目标）
        /// </summary>
        private static AbilityM_AuroraDragonFireBall FindByMultiKeys(byte K_BossType, string K_SceneType) {
        ...
        private static void LoadCsvAndCacheAll(Action onLoadComplete) {
            ...
            AssetCsvManager.GetAbilityABAllCsvRows("AbilityM_AuroraDragonFireBall", allRows => {
                if (allRows == null || allRows.Count == 0) {
                    Debug.LogError($"全量加载 CSV 失败：AbilityM_AuroraDragonFireBall");
                ...
                foreach (var row in allRows) {
                    ...
                    if (!byte.TryParse(idStr, out byte rowId)) {
                        Debug.LogError($"CSV 行 ID 格式错误：AbilityM_AuroraDragonFireBall → {idStr}");
                        continue;
                    }
                    if (idCache.ContainsKey(rowId)) {
                        Debug.LogError($"CSV AbilityM_AuroraDragonFireBall 行 ID重复：{rowId}（已跳过）");
                        continue;
                    }
                    var rowSign = AssetCsvManager.GetCsvRowValue(row, "RowSign");
                    var instance = new AbilityM_AuroraDragonFireBall();
                    ...
                    instance.M_CreateHitEffectTime = AssetCsvManager.ParseValue<float>(AssetCsvManager.GetCsvRowValue(row, "M_CreateHitEffectTime"));
                ...
                IsCsvLoaded = true;
                #if UNITY_EDITOR
                Debug.Log($"CSV全量加载完成：AbilityM_AuroraDragonFireBall → 缓存 {idCache.Count}个实例  （耗时 {Time.realtimeSinceStartup - time:F2}秒）");
                #endif
                onLoadComplete?.Invoke();
            });
        }
        ...
        /// </summary>
        private void CopyInstanceData(AbilityM_AuroraDragonFireBall source) {
            // 拷贝主键字段（K_XXX）
            ...
            this.M_CreateHitEffectTime = source.M_CreateHitEffectTime;
        ...
        /// 按主键创建实例（自动赋值K_主键字段）
        /// </summary>
        public static AbilityM_AuroraDragonFireBall CreateForKey(byte k_BossType, string k_SceneType) {
            // 缓存已加载时直接查询并返回缓存实例
            if (IsCsvLoaded) {
        ...
        /// 按ID创建实例（自动赋值K_ID）
        /// </summary>
        public static AbilityM_AuroraDragonFireBall CreateForID(byte id) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        public static AbilityM_AuroraDragonFireBall CreateForSign(string sign) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        private static AbilityM_AuroraDragonFireBall CreateSO() {
#if UNITY_EDITOR
           var url = $"{AssetURL.AbilitySO}SO/AB/AbilityM_AuroraDragonFireBall.asset";
           var so = AssetDatabase.LoadAssetAtPath<AbilityM_AuroraDragonFireBall>(url);
           if (so != null && so.IsEditorUseSO) {
              so.IsSetCsvData = true;
              return so;
           }
#endif
           return new AbilityM_AuroraDragonFireBall();
        }
    }
}
