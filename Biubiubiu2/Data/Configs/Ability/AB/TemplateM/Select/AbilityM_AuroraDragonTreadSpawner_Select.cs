...
namespace Sofunny.BiuBiuBiu2.Data {
    public partial class AbilityM_AuroraDragonTreadSpawner {
        public const string AbilityTypeID = AbilityConfig.AB_AuroraDragonTreadSpawner;
        public const ushort ConfigId = AbilityConfig.AuroraDragonTreadSpawner;
        private static Dictionary<byte, AbilityM_AuroraDragonTreadSpawner> idCache = new Dictionary<byte, AbilityM_AuroraDragonTreadSpawner>();
        private static Dictionary<string, AbilityM_AuroraDragonTreadSpawner> signCache = new Dictionary<string, AbilityM_AuroraDragonTreadSpawner>();
        private static List<AbilityM_AuroraDragonTreadSpawner> loadingCallbacks = new List<AbilityM_AuroraDragonTreadSpawner>();
        ...
        /// 格式：ID_主键值组合 = ID（如AbilityM_AuroraDragonTreadSpawner.ID_1）
        /// </summary>
        ...
        /// <summary>
        /// ID查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_AuroraDragonTreadSpawner.ID_1）后调用Select()
        /// </summary>
        [NonSerialized]
        public byte K_ID = 0;
        /// <summary>
        /// Sign 查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_AuroraDragonTreadSpawner.Sign_XXX）后调用Select()
        /// </summary>
        [NonSerialized]
        ...
        public static void ClearCache() {
            ...
            IsCsvLoaded = false;
            isLoading = false;
            Debug.Log("已清除 AbilityM_AuroraDragonTreadSpawner 实例缓存");
        }
        ...
        private void DoSelectFromCache() {
            AbilityM_AuroraDragonTreadSpawner target = null;
            // 1. 优先ID查询（性能最优，无遍历）
            if (this.K_ID != 0) {
            ...
            if (target != null) {
                ...
                Debug.LogError($"AbilityM_AuroraDragonTreadSpawner 查询未找到：K_BossType={this.K_BossType}, K_SceneType={this.K_SceneType} 或 K_ID={this.K_ID} 或 K_RowSign={this.K_RowSign}");
            }
            callBack?.Invoke();
            callBack = null;
        }
        ...
        /// 多K字段匹配逻辑（遍历实例缓存找目标）
        /// </summary>
        private static AbilityM_AuroraDragonTreadSpawner FindByMultiKeys(byte K_BossType, string K_SceneType) {
        ...
        private static void LoadCsvAndCacheAll(Action onLoadComplete) {
            ...
            AssetCsvManager.GetAbilityABAllCsvRows("AbilityM_AuroraDragonTreadSpawner", allRows => {
                if (allRows == null || allRows.Count == 0) {
                    Debug.LogError($"全量加载 CSV 失败：AbilityM_AuroraDragonTreadSpawner");
                ...
                foreach (var row in allRows) {
                    ...
                    if (!byte.TryParse(idStr, out byte rowId)) {
                        Debug.LogError($"CSV 行 ID 格式错误：AbilityM_AuroraDragonTreadSpawner → {idStr}");
                        continue;
                    }
                    if (idCache.ContainsKey(rowId)) {
                        Debug.LogError($"CSV AbilityM_AuroraDragonTreadSpawner 行 ID重复：{rowId}（已跳过）");
                        continue;
                    }
                    var rowSign = AssetCsvManager.GetCsvRowValue(row, "RowSign");
                    var instance = new AbilityM_AuroraDragonTreadSpawner();
                    ...
                    instance.M_TreadAttackNum = AssetCsvManager.ParseValue<int>(AssetCsvManager.GetCsvRowValue(row, "M_TreadAttackNum"));
                    instance.M_TreadAttackTime = AssetCsvManager.ParseValue<float>(AssetCsvManager.GetCsvRowValue(row, "M_TreadAttackTime"));
                    instance.M_TreadFireAttacKTime = AssetCsvManager.ParseValue<float>(AssetCsvManager.GetCsvRowValue(row, "M_TreadFireAttacKTime"));
                ...
                IsCsvLoaded = true;
                #if UNITY_EDITOR
                Debug.Log($"CSV全量加载完成：AbilityM_AuroraDragonTreadSpawner → 缓存 {idCache.Count}个实例  （耗时 {Time.realtimeSinceStartup - time:F2}秒）");
                #endif
                onLoadComplete?.Invoke();
            });
        }
        ...
        /// </summary>
        private void CopyInstanceData(AbilityM_AuroraDragonTreadSpawner source) {
            // 拷贝主键字段（K_XXX）
            ...
            this.M_TreadAttackNum = source.M_TreadAttackNum;
            this.M_TreadAttackTime = source.M_TreadAttackTime;
            this.M_TreadFireAttacKTime = source.M_TreadFireAttacKTime;
        }

        /// <summary>
        /// 按主键创建实例（自动赋值K_主键字段）
        /// </summary>
        public static AbilityM_AuroraDragonTreadSpawner CreateForKey(byte k_BossType, string k_SceneType) {
            // 缓存已加载时直接查询并返回缓存实例
            if (IsCsvLoaded) {
        ...
        /// 按ID创建实例（自动赋值K_ID）
        /// </summary>
        public static AbilityM_AuroraDragonTreadSpawner CreateForID(byte id) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        public static AbilityM_AuroraDragonTreadSpawner CreateForSign(string sign) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        private static AbilityM_AuroraDragonTreadSpawner CreateSO() {
#if UNITY_EDITOR
           var url = $"{AssetURL.AbilitySO}SO/AB/AbilityM_AuroraDragonTreadSpawner.asset";
           var so = AssetDatabase.LoadAssetAtPath<AbilityM_AuroraDragonTreadSpawner>(url);
           if (so != null && so.IsEditorUseSO) {
              so.IsSetCsvData = true;
              return so;
           }
#endif
           return new AbilityM_AuroraDragonTreadSpawner();
        }
    }
}
