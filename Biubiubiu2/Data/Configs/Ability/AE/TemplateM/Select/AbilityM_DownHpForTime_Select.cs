...
namespace Sofunny.BiuBiuBiu2.Data {
    public partial class AbilityM_DownHpForTime {
        public const string AbilityTypeID = AbilityEffectConfig.AE_DownHpForTime;
        public const ushort ConfigId = AbilityEffectConfig.DownHpForTime;
        private static Dictionary<byte, AbilityM_DownHpForTime> idCache = new Dictionary<byte, AbilityM_DownHpForTime>();
        private static Dictionary<string, AbilityM_DownHpForTime> signCache = new Dictionary<string, AbilityM_DownHpForTime>();
        private static List<AbilityM_DownHpForTime> loadingCallbacks = new List<AbilityM_DownHpForTime>();
        ...
        /// 格式：ID_主键值组合 = ID（如AbilityM_DownHpForTime.ID_1）
        /// </summary>
        public const byte ID_FireBallDownHpForTime = 1;

        /// <summary>
        /// CSV Sign 常量映射（自动生成，对应CSV的RowSign列）
        /// 格式：Sign_XXX
        /// </summary>
        public const string Sign_FireBallDownHpForTime = "FireBallDownHpForTime";

        /// <summary>
        /// ID查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_DownHpForTime.ID_1）后调用Select()
        /// </summary>
        [NonSerialized]
        public byte K_ID = 0;
        /// <summary>
        /// Sign 查询主键（自动生成，优先于其他主键）
        /// 用法：赋值ID常量（如AbilityM_DownHpForTime.Sign_XXX）后调用Select()
        /// </summary>
        [NonSerialized]
        ...
        public static void ClearCache() {
            ...
            IsCsvLoaded = false;
            isLoading = false;
            Debug.Log("已清除 AbilityM_DownHpForTime 实例缓存");
        }
        ...
        private void DoSelectFromCache() {
            AbilityM_DownHpForTime target = null;
            // 1. 优先ID查询（性能最优，无遍历）
            if (this.K_ID != 0) {
            ...
            if (target != null) {
                ...
                Debug.LogError($"AbilityM_DownHpForTime 查询未找到：K_Sign={this.K_Sign} 或 K_ID={this.K_ID} 或 K_RowSign={this.K_RowSign}");
            }
            callBack?.Invoke();
            callBack = null;
        }
        ...
        /// 多K字段匹配逻辑（遍历实例缓存找目标）
        /// </summary>
        private static AbilityM_DownHpForTime FindByMultiKeys(string K_Sign) {
        ...
        private static void LoadCsvAndCacheAll(Action onLoadComplete) {
            ...
            AssetCsvManager.GetAbilityAEAllCsvRows("AbilityM_DownHpForTime", allRows => {
                if (allRows == null || allRows.Count == 0) {
                    Debug.LogError($"全量加载 CSV 失败：AbilityM_DownHpForTime");
                ...
                foreach (var row in allRows) {
                    ...
                    if (!byte.TryParse(idStr, out byte rowId)) {
                        Debug.LogError($"CSV 行 ID 格式错误：AbilityM_DownHpForTime → {idStr}");
                        continue;
                    }
                    if (idCache.ContainsKey(rowId)) {
                        Debug.LogError($"CSV AbilityM_DownHpForTime 行 ID重复：{rowId}（已跳过）");
                        continue;
                    }
                    var rowSign = AssetCsvManager.GetCsvRowValue(row, "RowSign");
                    var instance = new AbilityM_DownHpForTime();
                ...
                IsCsvLoaded = true;
                #if UNITY_EDITOR
                Debug.Log($"CSV全量加载完成：AbilityM_DownHpForTime → 缓存 {idCache.Count}个实例  （耗时 {Time.realtimeSinceStartup - time:F2}秒）");
                #endif
                onLoadComplete?.Invoke();
            });
        }
        ...
        /// </summary>
        private void CopyInstanceData(AbilityM_DownHpForTime source) {
            // 拷贝主键字段（K_XXX）
        ...
        /// 按主键创建实例（自动赋值K_主键字段）
        /// </summary>
        public static AbilityM_DownHpForTime CreateForKey(string k_Sign) {
            // 缓存已加载时直接查询并返回缓存实例
            if (IsCsvLoaded) {
        ...
        /// 按ID创建实例（自动赋值K_ID）
        /// </summary>
        public static AbilityM_DownHpForTime CreateForID(byte id) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        public static AbilityM_DownHpForTime CreateForSign(string sign) {
            // 缓存已加载时直接返回缓存实例
        ...
        /// </summary>
        private static AbilityM_DownHpForTime CreateSO() {
#if UNITY_EDITOR
           var url = $"{AssetURL.AbilitySO}SO/AE/AbilityM_DownHpForTime.asset";
           var so = AssetDatabase.LoadAssetAtPath<AbilityM_DownHpForTime>(url);
           if (so != null && so.IsEditorUseSO) {
              so.IsSetCsvData = true;
              return so;
           }
#endif
           return new AbilityM_DownHpForTime();
        }
    }
}
