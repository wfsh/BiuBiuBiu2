...
namespace Sofunny.BiuBiuBiu2.ServerGamePlay {
    public class Condition_GPOMTypeDead : EventDirectorCondition {
        public struct InitData : IEventDirectorData {
            public int GpoMTypeId;
            public int DeadCount;
            public void Serialize (string value) {
                var arr = value.Split('&');
                GpoMTypeId = int.Parse(arr[0]);
        ...
        private void OnSetIsDead(ISystemMsg body, SE_GPO.Event_SetIsDead ent) {
            if (useMData.GpoMTypeId == 0 || ent.DeadGpo.GetGpoMTypeID() == useMData.GpoMTypeId) {
                deadCount++;
